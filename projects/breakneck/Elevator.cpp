...
Elevator::Elevator(int id, int startFloor, int minFloor, int maxFloor)
    : mJoinThread(false), mThread(&Elevator::start, this), mId(id),
      mFloorNum(startFloor), mMinFloor(minFloor), mMaxFloor(maxFloor),
...
std::optional<Elevator::FloorDiffType> Elevator::addFloor(int floorNum,
                                                          bool now) {
...
Elevator::addFloorIfInPath(int destFloor) {
  ...
  if (mDirection == Direction::Stand || destFloor == mMinFloor ||
      destFloor == mMaxFloor ||
